        ...
        <div className={styles['view-exercise-log']}>
            <div className={styles.content}>
                ...
                <div className={styles.targetMuscles}>
                    ...
                    {logData && logData?.data.targetMuscles && logData?.data.targetMuscles.length > 0 ? logData?.data.targetMuscles.map((item, index)=>(
                    ...
                    {logData && logData?.data.sets && logData?.data.sets.length > 0 ? logData?.data.sets.map((set, index)=>(
                        <div className={styles.set} key={'Set-'+index}>
                            <div className={styles.setHeader}>
                                <p className={styles.setNo}>Set {index+1}</p>
                                <p>{set.fields.every(item=>item.isCompleted) ? <img style={{width: '20px', height: '20px'}} src={IconLibrary.Checkmark} alt='' /> : null}</p>
                            </div>
                            <div className={styles.setFields}>
                                {set.fields && set.fields.length > 0 ? set.fields.map((field, index)=><div key={'Field-'+index+set.order} className={styles.field}>{field.name}: {field.value}/{field.target} {field.unit.shortLabel}</div>) : null}
                            </div>
                        </div>
                    )) : <p>No sets</p>}
                </div>
