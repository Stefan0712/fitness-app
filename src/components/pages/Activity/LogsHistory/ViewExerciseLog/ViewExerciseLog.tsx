...
const ViewExerciseLog: React.FC<ViewExerciseLogProps> = ({logData, closeLog}) => {
    ...
    return ( 
        <div className={styles['view-exercise-log']}>
            ...
            <p className={styles.timestamp}>{getDateFromTimestamp(logData.timestamp)} at {getHourFromTimestamp(logData.timestamp)} for {logData.duration}</p>
            ...
            <div className={styles.sets}>
                {logData && logData.sets && logData.sets.length > 0 ? logData.sets.map((set, index)=>(
                    <div className={styles.set} key={'Set-'+index}>
                        <p className={styles.setNo}>{index+1}</p>
                        <div className={styles.setFields}>
                            {set.fields && set.fields.length > 0 ? set.fields.map((field, index)=><div key={'Field-'+index+set.order} className={styles.field}>{field.value}/{field.target} {field.unit.shortLabel}</div>) : null}
                        </div>
