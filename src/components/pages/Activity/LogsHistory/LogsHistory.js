        ...
        <div className={styles['logs-history']}>
                ...
                {logs && logs.length > 0 ? logs.map((item, index) => (
                        <div className={styles['log-body']} key={'log-' + index} onClick={()=>setOpenedLog(item)}>
                            <img className={styles['log-icon']} src={item.type === 'food' ? IconLibrary.Food : item.type === 'exercise' ? IconLibrary.Exercise : item.type === 'workout' ? IconLibrary.Dumbbell : IconLibrary.Circle} alt='' />
